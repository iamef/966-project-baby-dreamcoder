...
#     """
    ...
    return get_filtered_funcs_probabilities(is_right_type, weights_by_type)


def get_function_probabilities_by_output_type(output_type: type,
                                 weights_by_type: dict[type, dict[Tuple[type, ...]], float]
                                 ) -> List[Tuple[callable, float]]:
    return get_filtered_funcs_probabilities(
        # the isfunction is not necessary, but redundancy is nice to avoid errors I guess
